...
export function RapportageWeergave({ boekingen = MOCK_BOOKINGS }: { boekingen?: Booking[] }) {
  const [periode, setPeriode] = useState<Periode>("maand");
  const [referentieDatum, setReferentieDatum] = useState(new Date());
  ...
  }, [periode, referentieDatum, boekingen]);
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        <div
          key={`${periode}-${referentieDatum.getTime()}`}
          className="bdz-fade rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm print:rounded-none print:border-0 print:shadow-none"
        >
          <style>{`
            @keyframes bdz-fade {
            ...
            .bdz-fade { animation: bdz-fade 1.2s cubic-bezier(0.16, 1, 0.3, 1) both; }
            @media (prefers-reduced-motion: reduce) { .bdz-fade { animation: none; } }
          `}</style>
