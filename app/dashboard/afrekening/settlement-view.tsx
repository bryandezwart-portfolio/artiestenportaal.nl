...
function ArtistCombobox({
  ...
  const [query, setQuery] = useState(selectedArtistName);
  const [isOpen, setIsOpen] = useState(false);
  const [highlighted, setHighlighted] = useState(0);
  const wrapperRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    setQuery(selectedArtistName);
  }, [selectedArtistName]);
  ...
  const filtered = useMemo(() => {
    ...
    if (!q || q === selectedArtistName.toLowerCase()) return artists;
    return artists.filter((a) => a.name.toLowerCase().includes(q));
  }, [artists, query, selectedArtistName]);
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      if (filtered[highlighted]) selectArtist(filtered[highlighted]);
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      />
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full max-h-56 overflow-y-auto bg-surface border border-line rounded-lg shadow-card">
          {filtered.length === 0 && (
          ...
          {filtered.map((a, i) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-[13.5px] transition ${
                a.id === selectedArtistId ? "text-accent font-medium" : "text-ink"
